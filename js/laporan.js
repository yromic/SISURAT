(function initLaporanPage(global) {
  ...
  function nextPreviewPage() {
    const totalPages = Math.ceil(
      _lastPreviewData.length / state.previewPageSize,
    );
  ...
  function _renderPreviewPage() {
    ...
    const uniqueTables = [
      ...new Set(data.map((r) => r._table).filter(Boolean)),
    ];
    ...
    const cols = isMixed
      ? CSV_COLUMNS_MIXED
      : CSV_COLUMNS[uniqueTables[0]] || CSV_COLUMNS["db_piagam"];
    ...
    if (state.sortKey) {
      sorted.sort((a, b) => {
        let va = String(a[state.sortKey] ?? "")
          .trim()
          .toLowerCase();
        let vb = String(b[state.sortKey] ?? "")
          .trim()
          .toLowerCase();
        ...
        return (
          va.localeCompare(vb, "id", { sensitivity: "base" }) * state.sortDir
        );
      });
    }
      ...
      .map((row) => {
        ...
        if (isMixed) {
          ...
          <tr class="tr-preview">
            ...
            ${cols
              .slice(1)
              .map((c) => `<td class="td-preview">${row[c.key] || "—"}</td>`)
              .join("")}
          </tr>`;
        } else {
    ...
    if (pager) {
      pager.style.display =
        totalPages > 1 || total > pageSize ? "flex" : "none";
  ...
  function downloadCsv() {
    ...
    if (jenisVal) {
      ...
      SisuratApi.exportCsv(
        csvRows,
        `rekap_${TABLE_LABELS[jenisVal]}_${today()}.csv`,
      );
    } else {
  ...
  async function downloadPdf() {
    ...
    for (const table of selectedTables) {
      ...
      if (
        yStart > 160 &&
        selectedTables.indexOf(table) < selectedTables.length - 1
      ) {
        doc.addPage();
        yStart = 14;
      }
